...
# pygame.mixer.music.load(os.path.join('c:\prj\prg', 'game.mp3'))
# pygame.mixer.music.set_volume(0.4)
player_img = pygame.image.load(os.path.join(img_folder, 'badge_01.png'))
player2_img = pygame.image.load(os.path.join(img_folder, 'badge_1.png'))
...
while running:
    score+=1/40
    if game_over:
        player2.rect.center = (WIDTH , HEIGHT )
        score+=0
    ...
    if hits:
        player.kill()
        player.killed=True
    hits = pygame.sprite.spritecollide(player2, mobs, False)
    if hits:
        player2.kill()
        player2.killed=True
    ...
    if player.killed & player2.killed:
        print("GAME-OVER!")
        game_over = True
    # Рендеринг
    screen.fill(BLUE)
